...
to initialize_turtles
  ...
  ask turtles [ set quarantine? false]
end
...
to go_covid
  ...
  if count turtles with [sick? = true ] >= 10 [ if quarantine = "quarantine_student" [ ask turtles with [student? = true] [ set quarantine? true]]
  if quarantine = "quarantine_worker"  [ ask turtles with [worker? = true] [set quarantine? true]]
  if quarantine = "quarantine_retired" [ ask turtles with [retired? = true] [set quarantine? true]]
  if quarantine = "quarantine_worker+retired" [ ask turtles with [worker? = true or retired? = true] [ set quarantine? true]]
  if quarantine = "quarantine_retired+student" [ ask turtles with [student? = true or retired? = true] [set quarantine? true]]
  if quarantine = "quarantine_student+worker" [ ask turtles with [student? = true or worker? = true] [set quarantine? true]]
  if quarantine = "all" [ ask turtles[ set quarantine? true]]]
  ask turtles [if quarantine? = false [
    move]]
  ask turtles [if quarantine? = true and notp? = true and random 100 < 50 [ set paranoid? true move]]
  ask turtles [if quarantine? = true  and paranoid? = 0
  [slow_move]]
  ask turtles [if sick? = 0 [move]]
  ask turtles
  [
    if sick? = true [if infect-count <= 2  [infect]]
    sickness
    recover-die
  ]
  if count turtles with [ sick? = true ] = 0 [stop]
  ask turtles with [student? = true] [if quarantine? = true [ set neg_student neg_student + 0.75]]
  ask turtles with [worker? = true] [if quarantine? = true [ set neg_worker neg_worker + 1]]
  ask turtles with [retired? = true] [if quarantine? = true [ set neg_retired neg_retired + 0.5]]
  tick
end
...
to move
 ...
 if student? = true and infect-count <= 2
 [ rt random 80
  lt random 80
    fd 1]

  if worker? = true and infect-count <= 2
  [ rt random 80
  lt random 80
    fd 1]

  if retired? = true and infect-count <= 2
  [ rt random 80
  lt random 80
    fd 0.75]
...
to infect
  if any? other turtles-here with [sick? = false] [ set infect-count infect-count + 1
 ask other turtles-here with [ sick? = false  ] [ if random 100 <= infectious_rate
      [ get-sick ] ]]
...
quarantine
quarantine
"quarantine_worker" "quarantine_retired" "quarantine_student" "none" "quarantine_worker+retired" "quarantine_retired+student" "quarantine_student+worker" "all"
0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [ sick? = true ]"
